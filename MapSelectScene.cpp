...
void MapSelectScene::Initialize()
{
	...
	mMap1->Initialize(Framework::GetDevice());
	mMap1->SetPosition(0, 3.0f, 0);
	mMap1->SetScale(0.05f, 0.05f, 0.05f);
	mMap1->SetRotation(0, 90.0f, 0);

	mMap2->Initialize(Framework::GetDevice());
	mMap2->SetPosition(0, -5.0f, 0);
	mMap2->SetScale(0.05f, 0.05f, 0.05f);
	mMap2->SetRotation(0, 90.0f, 0);

	mGameObjects.push_back(mMap1);
	mGameObjects.push_back(mMap2);
}
...
void MapSelectScene::Update(float dt)
{
	...
	if (Input::GetKeyDown(eKeyCode::LButton) && mMap1 && mMap2)
	{
		...
		float dist = 0.0f;
		DirectX::BoundingBox Map1_aabb = mMap1->GetWorldAABB();
